...
description: End-to-end TFS development workflow for pulling reviewed user stories from a saved TFS query, using a user-provided local workspace/repository, preparing a temporary feature branch from dev, guiding code development, submitting a pull request into dev, enabling auto-complete with source-branch deletion, and optionally marking the user story resolved after completion. Use when the user asks to pull requirements from TFS, start development from a TFS user story, find a local workspace for a requirement, finish development and submit a PR, or perform the full TFS-to-code-to-PR workflow.
---
...
- Before any requirement code exploration or edits, update each involved repository from the latest `origin/dev`; use `dev` as the default base branch.
- For each repo that needs changes, start from `origin/dev` and create a temporary branch named `feature/<workItemId>-<tfsAlias>`, for example `feature/1551572-yangtao`.
- Never commit directly to `dev` and never push directly to `dev`.
...
1. **Select requirement**
   ...
   - Otherwise execute the configured saved query with `scripts/Get-TfsSavedQueryWorkItems.ps1`, which defaults to `WorkItemType=用户情景` and `State=已评审`.
...
3. **Collect workspace**
   ...
   - If a path is not a git repo or does not point to `dev.tellhowsoft.com`, stop and ask for the correct path.

4. **Update from dev**
   - Before reading code for implementation, run the branch preparation against the latest `origin/dev`.
   - If the repo is already dirty, inspect the changes first and preserve them; do not overwrite unrelated local work.

5. **Prepare branch**
   - Run `scripts/Start-TfsFeatureBranch.ps1` for each repo.
   - Branch format must be `feature/<workItemId>-<tfsAlias>`.
   - Base branch must be `origin/dev`.
...
7. **Submit PR**
   - Use `tfs-git-pr` to stage intended files, commit, push, create PR to `dev`, set auto-complete, delete source branch on completion, and link the work item.
...
8. **Resolve requirement**
   - Only after the user confirms the requirement is complete, update the user story state to `已解决` with `scripts/Set-TfsWorkItemState.ps1`.
